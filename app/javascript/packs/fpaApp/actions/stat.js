...
export const calculateMeans = players => {
  return dispatch => {
    ...
    for (let i = 0; i < players[0].player_stats.stats.stat.length; i++) {
      if (
        players[0].player_stats.stats.stat[i].stat_id === "9004003" ||
        players[0].player_stats.stats.stat[i].stat_id === "9007006"
      ) {
        // do nothing
      } else {
        means[players[0].player_stats.stats.stat[i].stat_id] =
          parseFloat(players[0].player_stats.stats.stat[i].value) || 0;
      }
    }
    ...
    for (let k = 1; k < players.length; k++) {
      for (let j = 0; j < players[k].player_stats.stats.stat.length; j++) {
        if (
          players[k].player_stats.stats.stat[j].stat_id === "9004003" ||
          players[k].player_stats.stats.stat[j].stat_id === "9007006"
        ) {
          // do nothing
        } else {
          means[players[k].player_stats.stats.stat[j].stat_id] +=
            parseFloat(players[k].player_stats.stats.stat[j].value) || 0;
        }
      }
    }
  ...
  return dispatch => {
    ...
    for (let i = 0; i < players[0].player_stats.stats.stat.length; i++) {
      if (
        ...
        stdDeviations[players[0].player_stats.stats.stat[i].stat_id] = Math.pow(
          (parseFloat(players[0].player_stats.stats.stat[i].value) || 0) -
            means[players[0].player_stats.stats.stat[i].stat_id],
          2
        );
    ...
    for (let k = 1; k < players.length; k++) {
      for (let j = 0; j < players[k].player_stats.stats.stat.length; j++) {
        if (
          ...
          stdDeviations[
            players[k].player_stats.stats.stat[j].stat_id
          ] += Math.pow(
            (parseFloat(players[k].player_stats.stats.stat[j].value) || 0) -
              means[players[k].player_stats.stats.stat[j].stat_id],
            2
...
export const determineCategoryLabels = player => {
  return dispatch => {
    ...
    let categoryLabels = ["Rank", "Name", "Owner"];
    ...
    for (let i = 0; i < player.player_stats.stats.stat.length; i++) {
      categoryLabels.push(
        playerStatKeys[player.player_stats.stats.stat[i].stat_id]
      );
    }

    // If succesfull, store our results in state
    categoryLabels.length > 0
      ? dispatch(setCategoryLabels(categoryLabels))
      : "";
  };
};
