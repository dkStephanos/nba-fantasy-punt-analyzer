...
export const getUserTeamPlayers = (players, teamKey) => {
  return dispatch => {
    const userTeamKey = middleware.getUserTeamKey();
    const userTeam = [];

    for (let i = 0; i < players.length; i++) {
      players[i].ownership.owner_team_key === userTeamKey
        ? userTeam.push(players[i])
        : "";
    }

    dispatch(setUserTeamPlayers(userTeam));
  };
};
  ...
  return dispatch => {
    ...
    for (let i = 0; i < players[0].player_stats.stats.stat.length; i++) {
      if (players[0].player_stats.stats.stat[i].stat_id === "9004003") {
        means["3"] =
          parseFloat(
            players[0].player_stats.stats.stat[i].value.split("/")[0]
          ) || 0;
        means["4"] =
          parseFloat(
            players[0].player_stats.stats.stat[i].value.split("/")[1]
          ) || 0;
    ...
    for (let k = 1; k < players.length; k++) {
      for (let j = 0; j < players[k].player_stats.stats.stat.length; j++) {
        if (players[k].player_stats.stats.stat[j].stat_id === "9004003") {
          means["3"] +=
            parseFloat(
              players[k].player_stats.stats.stat[j].value.split("/")[0]
            ) || 0;
          means["4"] +=
            parseFloat(
              players[k].player_stats.stats.stat[j].value.split("/")[1]
            ) || 0;
        } else if (
          players[0].player_stats.stats.stat[j].stat_id === "9007006"
        ) {
          means["6"] +=
            parseFloat(
              players[k].player_stats.stats.stat[j].value.split("/")[0]
    ...
    for (let i = 0; i < players.length; i++) {
      for (let j = 0; j < players[i].player_stats.stats.stat.length; j++) {
        switch (players[i].player_stats.stats.stat[j].stat_id) {
          case "5":
            fgPercentage =
              parseFloat(players[i].player_stats.stats.stat[j].value) || 0;
            break;
          case "8":
            ftPercentage =
              parseFloat(players[i].player_stats.stats.stat[j].value) || 0;
          ...
          case "9004003":
            fgAttempted =
              parseFloat(
                players[i].player_stats.stats.stat[j].value.split("/")[1]
              ) || 0;
            break;
          case "9007006":
            ftAttempted =
      ...
      fgImpact = Number(
        ((fgPercentage - averageFgPercentage) * fgAttempted).toFixed(3)
      );
      players[i].player_stats.stats.stat.push({
        stat_id: "1005",
        value: fgImpact
      });
      ftImpact = Number(
        ((ftPercentage - averageFtPercentage) * ftAttempted).toFixed(3)
      );
      players[i].player_stats.stats.stat.push({
        stat_id: "1008",
        value: ftImpact
...
export const calculateZScores = (players, means, stdDeviations) => {
  return dispatch => {
    ...
    for (let i = 0; i < players.length; i++) {
      for (let j = 0; j < players[i].player_stats.stats.stat.length; j++) {
        if (
          players[i].player_stats.stats.stat[j].stat_id === "9004003" ||
          players[i].player_stats.stats.stat[j].stat_id === "9007006"
        ) {
          // do nothing
        } else {
          players[i].player_stats.stats.stat[j]["zScore"] =
            ((parseFloat(players[i].player_stats.stats.stat[j].value) || 0) -
              means[players[i].player_stats.stats.stat[j].stat_id]) /
            stdDeviations[players[i].player_stats.stats.stat[j].stat_id];
        }
      }
    }
    // Finally, dispatch the setZScores action which will store the new player array in state
...
export const calculateAndSortPlayerRanksWithFilters = (players, filters) => {
  return dispatch => {
    ...
    if (filters.positions || filters.statuses) {
      ...
      for (let k = 0; k < playersLength; k++) {
        ...
        if (filters.positions) {
          ...
          matchesFilter = players[k].eligible_positions.position.some(
            position => {
    ...
    for (let i = 0; i < playersLength; i++) {
      ...
      for (
        let j = 0;
        j < filteredPlayers[i].player_stats.stats.stat.length;
        j++
      ) {
        if (
          filterKeys.includes(
            filteredPlayers[i].player_stats.stats.stat[j].stat_id
          ) ||
      ...
      // Once we've added all z-Scores, divide by the length to get the average zScore
      // and then store that result in the player object under the key 'rank'
      filteredPlayers[i].rank =
        rank / filteredPlayers[i].player_stats.stats.stat.length;
    }
